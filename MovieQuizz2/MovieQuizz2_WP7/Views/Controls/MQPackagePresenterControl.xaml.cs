...
namespace MovieQuizz2
{
    public partial class MQPackagePresenterControl : UserControl
    {
        ...
        {
            get { return (Visibility)GetValue(PosterVisibilityProperty); }
            set { SetValue(PosterVisibilityProperty, value); }
        }

        // Using a DependencyProperty as the backing store for PosterVisibility.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty PosterVisibilityProperty =
            DependencyProperty.Register("PosterVisibility", typeof(Visibility), typeof(MQPackagePresenterControl), new PropertyMetadata(Visibility.Visible, OnPosterVisibilityChanged));

        
        private static void OnPosterVisibilityChanged(DependencyObject dependencyObject,
               DependencyPropertyChangedEventArgs e)
        {
            MQPackagePresenterControl myUserControl = dependencyObject as MQPackagePresenterControl;
            myUserControl.OnPosterVisibilityChanged(e);
        }

        private void OnPosterVisibilityChanged(DependencyPropertyChangedEventArgs e)
        {
            this.ThreePosterControl.Visibility = (Visibility)e.NewValue;
        }
    }
}
